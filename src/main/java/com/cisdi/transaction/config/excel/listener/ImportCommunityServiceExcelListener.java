...
public class ImportCommunityServiceExcelListener extends AnalysisEventListener<CommunityServiceDTO> {
    ...
    private List<String> columns= Arrays.asList(
            "干部姓名","身份证号","工作单位","现任职务","职务层次","标签","职级","人员类别","政治面貌","在职状态","姓名","称谓","执业资格名称","执业证号","开办或所在机构名称","统一社会信用代码/注册号","经营（服务）范围","成立日期","注册地（国家）","注册地（省）","注册地（市）","经营（服务）地","机构类型","注册资本（金）或资金数额（出资额）（人民币万元）","经营状态","是否为机构股东（合伙人、所有人等）","个人认缴出资额或个人出资额（人民币万元）","个人认缴出资比例或个人出资比例（%）","入股（合伙）时间","是否在该机构中从业","所担任的职务名称","入职时间","是否与报告人所在单位（系统）直接发生过经济关系","备注","填报类型","年度","有无此类情况"
    );
