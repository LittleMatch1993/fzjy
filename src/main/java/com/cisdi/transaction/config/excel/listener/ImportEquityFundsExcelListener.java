...
public class ImportEquityFundsExcelListener extends AnalysisEventListener<EquityFundsDTO> {
    ...
    public ImportEquityFundsExcelListener(PrivateEquityService privateEquityService,BaseDTO baseDTO) {
    ...
    private void saveData() {
        ...
        if (list.size()>0){
            privateEquityService.saveBatchInvestmentInfo(list,baseDTO);
        }
        log.info("存储数据库成功！");
    }
}
