...
public class MinIoUtil {
     ...
     * @throws Exception
     */
    public String  downloadByMinio(MultipartFile file,
                                   String bucketName, String directory) throws Exception {
        if (!this.bucketExists(bucketName)) {
            this.makeBucket(bucketName);
        }
        InputStream inputStream = file.getInputStream();
        String fileName = file.getOriginalFilename();
        directory = DateUtil.format(new Date(), "yyyyMMddHHmmss")+"/"+Optional.ofNullable(directory).orElse("");
